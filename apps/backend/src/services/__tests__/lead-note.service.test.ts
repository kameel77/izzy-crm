...
describe("lead-note.service", () => {
  ...
  it("creates a lead note when actor is permitted", async () => {
    ...
    const createSpy = vi
      .spyOn(prisma.leadNote, "create")
      .mockResolvedValue(
        createdNote as unknown as Awaited<ReturnType<typeof prisma.leadNote.create>>,
      );
  ...
  it("lists lead notes using the requested sort order", async () => {
    ...
    const findManySpy = vi
      .spyOn(prisma.leadNote, "findMany")
      .mockResolvedValue(
        notes as unknown as Awaited<ReturnType<typeof prisma.leadNote.findMany>>,
      );
  ...
  it("defaults to descending order when no sort is provided", async () => {
    ...
    const findManySpy = vi
      .spyOn(prisma.leadNote, "findMany")
      .mockResolvedValue([] as unknown as Awaited<ReturnType<typeof prisma.leadNote.findMany>>);
